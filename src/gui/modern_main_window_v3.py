...
'''
    ...
    def show_help(self):
        ...
        help_window.title("Справка - auto2tesst")
        ...
        ctk.CTkLabel(
            help_container,
            text="🚀 auto2tesst - Octobrowser Script Builder",
            font=(ModernTheme.FONT['family'], 24, 'bold'),
            text_color=self.theme['accent_primary']
        ).pack(pady=(0, 10))

        ctk.CTkLabel(
            help_container,
            text="GUI-приложение конструктор для генерации Python скриптов автоматизации",
...
Octobrowser Script Builder - это визуальный конструктор для создания Python скриптов автоматизации браузера с использованием Octobrowser API и Playwright.

Приложение позволяет:
• Записывать действия с помощью Playwright Codegen
• Автоматически генерировать полноценные скрипты с Octobrowser API
• Работать с множественными профилями и CSV данными
• Управлять прокси, fingerprints и тегами профилей
• Запускать скрипты прямо из интерфейса

🎯 ОСНОВНЫЕ ВОЗМОЖНОСТИ

✅ Интеграция с Octobrowser API
   - Создание и управление профилями
   - Настройка прокси (HTTP, HTTPS, SOCKS5)
   - Генерация fingerprints
   - Управление тегами

✅ Работа с CSV данными
   - Загрузка данных из CSV файлов
   - Встраивание данных в скрипт
   - Автоматическая итерация по строкам
   - Каждая строка = отдельный профиль

✅ Playwright Integration
   - Запись действий через Playwright Codegen
   - Автоматическая очистка boilerplate кода
   - Подключение к Octobrowser через CDP
   - Поддержка popup окон (page1, page2, page3)

✅ Умные функции
   - Симуляция ввода текста (human typing)
   - Автоматические retry с прогрессивными задержками
   - Обработка динамических форм
   - Special commands (#pause, #scrolldown, #optional)

🔧 КАК ИСПОЛЬЗОВАТЬ

1. НАСТРОЙКА API:
   • Перейдите во вкладку "Octo API"
   • Вставьте ваш API токен из Octobrowser
   • Нажмите "Подключиться"

2. ЗАПИСЬ ДЕЙСТВИЙ:
   • Запустите Playwright Codegen в терминале:
     npx playwright codegen https://example.com
   • Выполните действия в браузере
   • Скопируйте сгенерированный код

3. ИМПОРТ КОДА:
   • Нажмите "ИМПОРТ" в главном окне
   • Код вставится из буфера обмена
   • Или вставьте код вручную в редактор

4. ЗАГРУЗКА CSV (опционально):
   • Подготовьте CSV файл с данными
   • Нажмите "Загрузить CSV"
   • Данные будут использоваться для каждой итерации

5. НАСТРОЙКА ГЕНЕРАЦИИ:
   • Выберите шаблон таймаутов или настройте вручную
   • Включите симуляцию ввода текста (рекомендуется)
   • Настройте задержки между действиями

6. ГЕНЕРАЦИЯ И ЗАПУСК:
   • Нажмите "ГЕНЕРИРОВАТЬ" для создания скрипта
   • Или сразу "ЗАПУСТИТЬ" (автоматически сгенерирует)
   • Наблюдайте за выполнением в логах

⚙️ НАСТРОЙКИ ТАЙМАУТОВ

• Таймаут кликов (рекомендуется 5-10 сек)
  - Время ожидания элементов на странице
  - Меньше = быстрее, но может не дождаться загрузки

• Таймаут навигации (рекомендуется 8-15 сек)
  - Время загрузки страниц
  - Зависит от скорости интернета

• Задержка между действиями (0.3-1 сек)
  - Пауза после каждого действия
  - Для стабильности и естественности

• Симуляция ввода текста (100ms)
  - Задержка между символами при вводе
  - 50-200ms для естественности
  - Помогает обойти bot detection

💡 SPECIAL COMMANDS

Используйте специальные команды в коде:

#pause5         - Пауза 5 секунд
#scrolldown     - Прокрутка вниз (100%)
#scrollmid      - Прокрутка в середину (50%)
#scrollup       - Прокрутка вверх (0%)
#optional       - Следующее действие необязательно (не будет ошибки)
...
auto2tesst v3.0 EPIC
Provider: smart_no_api
Python 3.8+

© 2025 auto2tesst Project
"""
        ...
        help_container.winfo_children()[-1].insert("1.0", info_text)
        help_container.winfo_children()[-1].configure(state="disabled")
